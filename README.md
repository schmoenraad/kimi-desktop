...
```bash
npm install -g @moonshot-ai/kimi-code@latest   # needs 0.23+
```
...
- Auto-starts the Kimi daemon on launch and injects its auth token, so reloads never
  hit the token wall
...
┌───────────▼─────────────┐
│  kimi web  (daemon)     │   official Kimi Code server + web UI
└───────────┬─────────────┘
            │
       api.kimi.com
```

The app spawns `kimi web --no-open`, waits for the port, reads the daemon's auth token
...
from the official daemon to Kimi.
...
- The Kimi daemon keeps running after you quit (it's shared with the CLI).
  Stop it with **Kimi Code → Stop Kimi Server** or `kimi server kill`.
- Built against Kimi Code **0.23.6**. It reads the daemon's REST API and
